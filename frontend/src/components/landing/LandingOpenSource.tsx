        ...
        {code.split('\n').map((line, i) => (
          <div key={i} className="flex">
            {line.startsWith('#') ? (
              ...
              <>
                <span className="text-kerpta dark:text-kerpta-400 select-none mr-2">$</span>
                <span>{line}</span>
              </>
            )}
          </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            {/* Licence */}
            <div className="rounded-2xl bg-kerpta-50 border border-kerpta-200 dark:bg-kerpta-600/5 dark:border-kerpta/20 p-5 text-center">
              <p className="text-xs text-gray-400 dark:text-slate-500 mb-1">Licence</p>
              <p className="text-lg font-bold text-kerpta-600 dark:text-kerpta-300">{license}</p>
          ...
          <div className="lg:col-span-3">
            <div className="relative space-y-6">
              ...
              <div className="absolute left-4 top-8 bottom-8 w-px bg-gradient-to-b from-kerpta/40 via-kerpta/20 to-transparent pointer-events-none" />

              {steps.map((step) => (
                <div key={step.step} className="relative pl-12">
                  {/* Numéro */}
                  <div className="absolute left-0 top-0 w-8 h-8 rounded-full bg-kerpta-50 border border-kerpta-200 dark:bg-kerpta-600/20 dark:border-kerpta/40 flex items-center justify-center">
                    <span className="text-xs font-bold text-kerpta-600 dark:text-kerpta-400">{step.step}</span>
                  </div>
